---
name: last30flames
description: "Research what's genuinely new about a topic in a recent time window. Use when the user asks what's new, recent, or trending on a subject and wants a source-grounded brief rather than a quick lookup. Gathers Firecrawl full-content web results plus Hacker News, Lobste.rs, Bluesky, and GitHub engagement signals in parallel; the agent synthesizes a cited brief from the numbered research context. Do not use for single quick lookups, current-weather-style facts, or anything that needs login-restricted sources (Reddit, X, TikTok, Instagram)."
argument-hint: 'last30flames AI coding agents | last30flames local LLM inference --days 7'
allowed-tools: Bash, Read
user-invocable: true
license: MIT
homepage: https://github.com/Laksh-star/last30flames
repository: https://github.com/Laksh-star/last30flames
upstream: https://github.com/firecrawl/last30flames
tags:
  - research
  - recency
  - firecrawl
...
> Codex note: this skill is harness-agnostic. The engine lives next to this
> `SKILL.md` in `scripts/` and is launched via `scripts/run.sh`, which
> self-locates, runs `bun install` on first run, and execs `scripts/index.ts`.
> No `CLAUDE_PLUGIN_ROOT` or Codex-specific env var is required — `run.sh`
> resolves its own directory through symlinks. The only binary it needs is `bun`.
> Optional env vars (`FIRECRAWL_API_KEY`, `GITHUB_TOKEN`) only raise rate limits;
> the skill runs keyless out of the box.
