---
name: last30flames
description: "Research what's genuinely new about a topic in a recent time window. Use when the user asks what's new, recent, or trending on a subject and wants a source-grounded, cited brief rather than a quick lookup. Gathers Firecrawl full-content web results plus Hacker News, Lobste.rs, Bluesky, and GitHub engagement signals in parallel; you synthesize a cited brief from the numbered research context. Skip for single quick lookups, current-weather-style facts, or anything needing login-restricted sources (Reddit, X, TikTok, Instagram)."
...
> Claude note: this skill is harness-agnostic. The engine lives next to this
...
> No `CLAUDE_PLUGIN_ROOT` or Claude-specific env var is required — `run.sh`
